...
$('.linebox-interedit').on('click','.btn-com',function(){
	//如果已经是选中状态，那么就直接结束
	if($(this).hasClass('btn-import'))return;
	var text = $(this).text();
	...
	if(text.indexOf("form")>=0){
		...
		appendHtml += ("<div class='line-interedit line-com'>"
				...
				+"<textarea name='queryParams["+appendIndex+"].description' id='' value='' placeholder='备注' style='width:31%'></textarea>"
				+"<i class='icon icon-delete f-l'></i>"
				+"</div>"
		);
		...
		appendHtml += ("<div class='line-interedit line-com'>"
				...
				+"<input placeholder='参数值' name='headerParams["+appendIndex+"].value' value='' style='width:20%' type='text'>"
...
$(function(){
	...
	$(".comfirm-interedit").find(".btn-com").click(function(){
		$.ajax({
			url:lemon.config.global.rootUrl+"/api/editSave",
